...
export const AuthProvider: React.FC<{ children: React.ReactNode }> = ({ children }) => {
  ...
  const register = async (userData: {
    ...
    try {
      // Map the fields to what your backend API expects
      // Based on your original implementation, backend expects:
      // phone, password, tg_id, agent_id (optional)
      // So we need to map:
      // - studentId becomes tg_id
      // - background becomes agent_id
      // - other fields may need to be sent if API accepts them
      ...
      if (userData.background && userData.background.trim() !== '') {
        payload.agent_id = userData.background;
      }

      // Include additional fields if your API accepts them
      // You may need to adjust based on what your backend actually expects
      ...
      const responseData = response.data;
      const { token, user: userDataFromResponse } = responseData.data;
