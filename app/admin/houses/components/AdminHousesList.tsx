// app/admin/houses/components/AdminHousesList.tsx
'use client';
...
interface AdminHousesListProps {
  onNavigate: (view: 'create' | 'edit' | 'view', houseId?: string) => void;
  refreshTrigger?: number;
}

const AdminHousesList = ({ onNavigate, refreshTrigger = 0 }: AdminHousesListProps) => {
  ...
  useEffect(() => {
    ...
    fetchHouses();
    fetchStats();
  }, [fetchHouses, fetchStats, fetchFilterOptions, refreshTrigger]);
                  ...
                  <motion.div initial={{ opacity: 0, y: 30 }} animate={{ opacity: 1, y: 0 }} transition={{ duration: 0.5, delay: index * 0.05 }} whileHover={{ y: -8 }}>
                    <Card sx={{ height: '100%', borderRadius: 3, overflow: 'hidden', backgroundColor: theme === 'dark' ? '#0f172a80' : 'white', backdropFilter: theme === 'dark' ? 'blur(10px)' : 'none' }}>
                      ...
                      <CardActions sx={{ p: 2, display: 'flex', gap: 1 }}>
                        <Button size="small" variant="outlined" startIcon={<Visibility />} onClick={() => onNavigate('view', house._id)} sx={{ flex: 1 }}>View</Button>
                        <Button size="small" variant="outlined" startIcon={<Edit />} onClick={() => onNavigate('edit', house._id)} sx={{ flex: 1 }}>Edit</Button>
                  ...
                  <Box key={house._id} component="tr" sx={{ display: 'table-row', borderBottom: 1, borderColor: 'divider', '&:hover': { bgcolor: theme === 'dark' ? '#1e293b' : '#f8fafc' } }}>
                    ...
                    <Box component="td" sx={{ p: 2 }}>
                      <Box sx={{ display: 'flex', gap: 1 }}>
                        <IconButton size="small" onClick={() => onNavigate('view', house._id)}><Visibility /></IconButton>
                        <IconButton size="small" onClick={() => onNavigate('edit', house._id)}><Edit /></IconButton>
...
export default AdminHousesList;